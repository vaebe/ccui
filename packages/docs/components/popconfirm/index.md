...
| okText              | string                                      | `'确 定'`   | 确定按钮文案（Ant 主名，未传时走 ConfigProvider locale）          |
| confirmText         | string                                      | `''`        | @deprecated 请改用 `okText`                                     |
| cancelText          | string                                      | `'取 消'`   | 取消按钮文案                                                    |
| okType              | `'primary' \| 'danger' \| 'default'`        | `'primary'` | 确定按钮类型（Ant 主名）                                         |
...
| open / v-model:open | boolean                                     | `undefined` | 受控显示状态（Ant 主名）                                         |
...
| update:open    | `(open: boolean)`    | v-model:open 同步（Ant 主名）                          |
| update:visible | `(visible: boolean)` | 同步触发的旧名，便于从 `v-model:visible` 渐进迁移      |
