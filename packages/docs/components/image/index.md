...
<template>
  ...
  <c-image src="https://invalid.example.com/y.png" :width="160" :height="120">
    <template #error>
      <div
        style="display: flex; align-items: center; justify-content: center; height: 100%; color: var(--ccui-color-error)"
      >
...
`classNames?: Record<RegionKey, string | undefined>` 与 `styles?: Record<RegionKey, CSSProperties | undefined>` 分别向语义区域注入 class 和内联样式，默认均为 `undefined`。`RegionKey` 可用值为 `root`、`image`、`previewMask`。
