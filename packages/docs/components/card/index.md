...
| classNames | `Record<string, string>`         | `undefined`           | 语义区域 class，支持 `root` / `header` / `cover` / `body`   |
| styles     | `Record<string, CSSProperties>`  | `undefined`           | 语义区域内联样式，支持 `root` / `header` / `cover` / `body` |
...
`classNames?: Record<RegionKey, string | undefined>` 与 `styles?: Record<RegionKey, CSSProperties | undefined>` 分别向语义区域注入 class 和内联样式，默认均为 `undefined`。`RegionKey` 可用值为 `root`、`header`、`cover`、`body`。
