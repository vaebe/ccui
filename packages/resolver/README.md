# @vaebe/unplugin-vue-components-ccui

`@vaebe/ccui` 组件按需自动导入解析器，搭配 [`unplugin-vue-components`](https://github.com/unplugin/unplugin-vue-components) 使用。
...
# npm
npm i -D @vaebe/unplugin-vue-components-ccui unplugin-vue-components
npm i @vaebe/ccui
```
...
模板组件名前缀。`@vaebe/ccui` 所有组件在源码里注册为 `name: 'C<Name>'`，所以默认是 `'C'`。
