...
1. Form：**shouldUpdate / validateDebounce / normalize 已交付**（48 个测试全通过）。剩余 `getValueProps`、状态图标、与录入组件的更深联动。
2. Table：**树形数据 childrenColumnName 已交付**（69 个测试全通过）。剩余虚拟滚动 / 远程数据协议 / 横向滚动固定表头。
