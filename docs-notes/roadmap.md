...
| M-B4  | Cascader    | `maxTagCount` + `max-tag-placeholder` slot + `maxTagTextLength` / 键盘导航 / `showSearch` 对象 `limit`/`sort` / `option` slot（替代 `optionRender`） / `popup` slot（替代 `popupRender`） / `searchOption` slot（替代 `showSearch.render`） | [ ]  |
