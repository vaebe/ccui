...
**✅ L-2.20 已交付（2026-05-15）**：1814 → 1823 测试（+9）。Tier L-2 错误边界（对标 ant `Alert.ErrorBoundary`）。要点：

- **新建 `packages/ccui/ui/error-boundary/`**，平铺独立 `<c-error-boundary>` / `import { ErrorBoundary }`。**不挂到 Alert 命名空间**。fallback 内部仍用 `<c-alert type="error">` 渲染默认错误视图，但 API 不对外暴露该绑定关系。
- **核心实现：`onErrorCaptured`**。setup 内 `onErrorCaptured((err, _instance, info) => { error.value = err; emit('error'); return false })`。返回 `false` 阻止错误向上冒泡，由本 boundary 兜底（不会再触发 app 级 errorHandler）。
- **捕获范围说明**（文档加 warning block）：同 React error boundary，**只捕同步渲染错误 + lifecycle hook 错误**；不捕异步 setTimeout / Promise reject，也不捕原生事件处理器内部的错。业务侧异步错误自己 try/catch，setError 模式手动控制。
- **fallback 三层优先级**：`#fallback` slot scope `{ error, errorInfo, reset }`（最高，完全替代）> `#message` / `#description` slot（替换 Alert 内的标题/描述）> `message` / `description` prop > `error.message` 兜底。
- **reset() 暴露**：`expose({ reset, error })` 让父组件通过 ref 重置错误态恢复 default slot。emit `reset` 配合事件钩子。
- **JSX 渲染**：本组件全 JSX 模板（feedback memory 已确认默认 JSX）。
- **顶层注册**：`packages/ccui/ui/vue-ccui.ts` 加 `ErrorBoundaryInstall` import + installs 数组 + 顶层 export。
- 文档 + sidebar：用户文档 4 demo（基本兜底 / 自定义 fallback / @error 日志上报 / reset 重试）+ 三表（参数 / 事件 / 插槽）+ 暴露方法表 + warning block 说明捕获范围；sidebar「反馈」组 Alert 后加入口。
- 测试 9 条：正常渲染 default slot / 抛错切 Alert fallback / message prop 渲染 / 未传 message 回退 error.message / message slot 优先 / fallback slot 替代 Alert / @error payload Error 实例 / reset() 恢复 default slot + @reset emit / 阻止错误向上冒泡（外层 errorHandler 不被调用）。每个测试用 `vi.spyOn(console, 'warn')` 静默 Vue 内部错误日志。
...
| L-2.20 | `ErrorBoundary`（`<c-error-boundary>`）                                                                              | `Alert.ErrorBoundary`                                                                         | 错误边界，Vue 内部用 `onErrorCaptured` 实现；**不挂到 Alert**                                                                                                             | [x]  |
