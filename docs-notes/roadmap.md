...
| --- | --- | --- | --- | --- |
| L-2.1 | `Textarea`（`<c-textarea>`） | `Input.TextArea` | 多行文本：`autoSize: bool \| { minRows, maxRows }` / `showCount` / `allowClear` / `count` / `maxLength`；底层 `<textarea>` | [ ] |
| L-2.2 | `InputSearch`（`<c-input-search>`） | `Input.Search` | 搜索框：`enterButton: bool \| string \| VNode` / `loading` / `@search` 事件 / `addon-after` slot | [ ] |
