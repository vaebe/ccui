...
| 类型     | 范围                                                                                                | 日期       | 理由 / 后续                                                                                          |
| -------- | --------------------------------------------------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------- |
| 撤回组件 | `Statistic` / `StatisticCountdown` / `StatisticTimer`                                               | 2026-05-16 | 业务不需要同步                                                                                       |
| 撤回组件 | `SkeletonAvatar` / `SkeletonButton` / `SkeletonImage` / `SkeletonInput`                             | 2026-05-16 | `Skeleton` / `SkeletonNode` 保留                                                                     |
| 撤回组件 | `DirectoryTree`                                                                                     | 2026-05-16 | `Tree` 保留                                                                                          |
| 撤回组件 | `DropdownButton`                                                                                    | 2026-05-16 | `Dropdown` / `Button` 保留                                                                           |
| 撤回组件 | `Tour`                                                                                              | 2026-05-16 | 业务不需要同步；roadmap M-B10 物理删，Tier M-B 由 10 批缩为 9 批                                     |
| 撤回组件 | `qr-code` / `list` / `list-item-meta` / `card-grid` / `form-error-list` / `error-boundary`          | 2026-05-16 | 业务不需要同步；L-2.10 改写为仅 CardMeta；L-2.11 / L-2.16 / L-2.20 物理删                            |
| 职责剥离 | Table 分页（`pagination` prop / `TablePaginationConfig` / `update:pagination` / `change` 首参）     | 2026-05-16 | 分页改为 c-pagination 外置组合；ant `pagination.placement` / `onChange(pagination, ...)` 等永久不补  |

> 下方 Tier 表中相关行的 `[x]` 仅作历史交付记录保留，不代表 v2 可用。已撤回组件的 demo / 文档 / sidebar / resolver 注册同步删除。后续若需补回，单独立 batch 评估。详细物理删步骤见各 Tier 段内 🗑 详细批次记录。
...
| M-A2  | `classNames` / `styles` 语义化 DOM 钩子（v5.18.0+）                                                                                                                       | 录入 12 + 展示 7（Table / Avatar / Badge / Card / Image / Tree / Calendar；List 已下线）+ 反馈 5（Modal / Drawer / Alert / Notification / Message）共 24 个     | +25          | 每组件至少 3 个语义化区域（root / popup.root / popup.listItem 等）可独立 className 注入                                                                                                             | [x]  |
...
| M-A4  | `prefix` / `suffixIcon` / `allowClear={ clearIcon }` / `loadingIcon` / `removeIcon` / `expandIcon` 自定义图标钩子（Vue：prop 接 string Iconify name；同名 slot 高优先级） | 10 个录入                                                                                                                                                       | +20          | 每个图标钩子同时支持 prop 形式（string / VNode）与同名 slot；slot 优先级高于 prop                                                                                                                   | [x]  |
| M-A5  | 命名差异统一表（21 项）加 Ant 别名层，旧名 deprecated                                                                                                                     | Button / Input / Modal / Drawer / Tooltip / Popover / Popconfirm / Form 7 个                                                                                    | +30          | `open` ↔ `visible`、`title` ↔ `content`、`htmlType` ↔ `nativeType`、`addonBefore/After` ↔ `prepend/append`、`mouseEnterDelay/LeaveDelay` ↔ `showAfter/hideAfter` 等同时支持，旧名 runtime warn 一次 | [x]  |
...
**🗑 2026-05-16 Table 移除分页职责**：c-table 不再承担分页 —— 删 `pagination` prop / `TablePaginationConfig` 类型 / 内部 `current` + `pageSize` state / `renderPagination()` UI / `update:pagination` 事件 / `change` 事件首参 `paginationState`（签名收紧为 `(filters, sorter, currentData)` 3 参）；同步删 `&__pagination` SCSS 块与 `import '../../pagination/src/pagination.scss'` 副作用引入；`packages/ccui/ui/table/index.ts` public types export 去掉 `TablePaginationConfig`。测试 `table.test.ts` 删 9 个 pagination 用例（true/false 渲染 / next-prev / 边界禁用 / total / pageSize 切 / 外部 prop 翻页 / 跨页 selectedRowKeys 保留 / sort 重置页）+ 改 2 个 emits change 用例为 3 参签名；用例 68 → 59，59/59 通过。文档同步删两 demo（Pagination / 受控分页）+ `TablePaginationConfig` 表 + `pagination` prop 行 + 事件表 `update:pagination` 行；`## change 事件全量追踪` demo 改 3 参签名；首段「能力覆盖」叙述去「分页」。**业务建议**：分页职责外置 —— 在 c-table 外组合 `<c-pagination>` + 在父组件 slice `dataSource`，table 只渲染当前页数据 + 排序 / 过滤（"render what you pass in" 单一职责）。c-pagination 组件本身未动；只是不再被 c-table 内部依赖。ant 端 `pagination.placement` / `onChange(pagination, ...)` 4 参签名 / `Ajax` demo 等与分页耦合的 API 永久标记为「ccui 设计不对齐 / 不补齐」。提交：`4509003`。

**🎉 Tier M-A 收官（2026-05-16）**：M-A1 ✓ / M-A2 ✓ / M-A3 ✓ / M-A4 ✓ / M-A5 ✓ —— 5 批全部完成。一次性给 50+ 组件横向补齐 v5/v6 通用能力（variant / classNames+styles / status / 图标钩子 / 命名差异 deprecated warn）。测试基线 1906 → 1996（净 +90），后续切到 Tier M-B 单组件长尾 + Tier L-3（命令式 / Hook API）。

---

**🧹 注释 cleanup 已交付（2026-05-16）**：1996 → 1996 测试（0 增量，纯注释清理）。Tier M-A 收官后扫 38 个文件（36 个 `*-types.ts` + `shared/hooks/use-semantic.ts` + `shared/hooks/use-icon.ts`），统一清理「对标 ant 版本元信息 / 路线图占位 / 批次标签」类 JSDoc 注释 —— 这些是提交时点的脚手架元数据（应放 git log / roadmap.md），不属于源码。三类处理：

<details>
<summary>详细要点</summary>

- **整段删 ~20 段**：纯版本+roadmap 占位（如 button `color × variant 矩阵（forward-compat 接收）/ SCSS 矩阵完整接入留 v2.x` 整块；button color/variant prop 的「Ant Design v5.21+ 新增，留 v2.x」单行注释；form 「对齐 Ant Design v5+ Rule.warningOnly」末行；notification / message 「L-3.5 新增」section 标签；12 个录入 variant interface alias 上的 「Ant Design v5.13+ 录入组件统一 variant 形态」单行 JSDoc）。
- **改写 ~80 段**：保留语义说明，删版本号 / 批次前缀。覆盖 24 组件 classNames+styles（M-A2）/ 12 组件 variant prop（M-A1 / M-A5）/ 11 处自定义图标钩子（M-A4）/ tooltip / popover 浮层细调段 / modal closable + forceRender 段 / splitter L-2.23 / layout L-2.21 / table L-2.12 / cascader + tree-select SHOW_* 常量末尾 roadmap 占位 / use-semantic / use-icon helper 头注释。
- **全部保留**：所有 `@deprecated` 标注 + 跟着的"请改用 xxx"建议（IDE 提示 + TS 警告契约）；复合 prop 语义说明（如 `loading: boolean | { delay, icon }` 的 boolean vs 对象延迟语义）；隐含约束 WHY（tooltip 「ccui 沿用本仓库 ms 统一口径」、Form 联动透传等）；正则 / 算法 / 边界条件 WHY；不带版本号的「对标 ant / 对齐 ant / Ant Design 主名」行为契约段。
...
// button-types.ts — loading 复合保留语义，删版本前缀
  /**
   * `loading` 复合配置：
...
// input-types.ts — variant prop 改写
- /**
...
// alert-types.ts — M-A2 classNames 改写
- /**
...
// tooltip-types.ts — 含 WHY 的边界条件保留
   /**
...
**不动 `.tsx` / `.scss` / 测试 / docs**：本批纯 types 层语义化注释清理；tsx 注释少且大多是边界 WHY（保留更安全），scss 注释属于设计令牌不在范围，测试和 docs 已交付不重新整理。测试基线 1996 → 1996（0 增量、0 失败，证明纯注释清理无副作用）。

</details>

---

**✅ M-A2 已交付（2026-05-16）**：1948 → 1996 测试（+48，99 文件不变）。Tier M-A 收官批 —— 24 个组件（录入 12 + 展示 7 + 反馈 5；List 已下线，roadmap 表 25 改 24）统一 Ant Design v5.18+ 风格的 `classNames` / `styles` 语义化 DOM 钩子，每组件至少 3 个语义化区域可独立 className / style 注入。要点：

<details>
<summary>详细要点</summary>

- **新建共享 helper**：`packages/ccui/ui/shared/hooks/use-semantic.ts` 暴露 `CcSemanticClasses` / `CcSemanticStyles` 两个 generic 类型 —— `Record<string, string | undefined>` / `Record<string, CSSProperties | undefined>`。所有 24 组件 types 文件统一 `import type { CcSemanticClasses, CcSemanticStyles } from '../../shared/hooks/use-semantic'` + 加同样的两个 prop（`classNames` / `styles`，default `undefined`）。**不强类型约束 region key 联合**（避免每组件单独定义 union 类型增加维护成本），可用 key 在每组件 prop 注释里列出。
- **注入范式**（利用 Vue 3 class binding 的 array 形态原生支持）：
  ```tsx
  <div class={[原 class, props.classNames?.root]} style={props.styles?.root}>
    <div class={[原 inner class, props.classNames?.<region>]} style={props.styles?.<region>}>
  ```
  原 class 是 object / string / array 都可以塞进 array，Vue normalize 自动合并；不修改原 class 逻辑，只追加 array 元素。这是 M-A2 与 M-A1 / M-A3 等批最显著差异 —— 不需要新 SCSS 或行为变更，纯**API surface 增强**。
- **24 组件 region key 表**：
  - **录入 12**：input（root / wrapper / input）/ inputNumber（root / input / controls）/ textarea（root / textarea / count）/ mentions（root / textarea / popup）/ select（root / selector / popup）/ autoComplete（root / input / popup）/ cascader（root / inputWrap / popup）/ treeSelect（root / inputWrap / popup）/ datePicker（root / input / popup）/ rangePicker（root / input / popup）/ timePicker（root / input / popup）/ colorPicker（root / trigger / popup）
  - **展示 7**：table（root / header / body / row）/ avatar（root / image / text）/ badge（root / dot / count）/ card（root / header / body / cover）/ image（root / image / previewMask）/ tree（root / node / switcher / label）/ calendar（root / header / body / cell）
  - **反馈 5**：modal（root / wrap / mask / header / body / footer）/ drawer（root / wrap / mask / header / body / footer）/ alert（root / icon / message / description）/ notification（root / icon / content / close）/ message（root / icon / content）
- **特殊处理 —— imperative API（Notification / Message）**：钩子在 options 上声明（`message.open({ classNames: { root: '...' } })` / `notification.open({ classNames: {...} })`），双 holder 路径（`use-message.ts` / `message.ts` 模块级 holder + `*-item.tsx`）都已透传到单条通知/消息的根 DOM（`.ccui-message__item` / `.ccui-notification__item`）。
- **特殊处理 —— Teleport（Modal / Drawer / popup-bearing 组件）**：钩子注入到 Teleport 容器内的真实根节点（不在 wrapper.element 上），测试断言改走 `document.body.querySelector('.ccui-modal.my-root')` 路径。popup 区域钩子定位在 `renderPopup()` / `buildPopup()` 返回的 panel 根 div 上（Teleport 之外的 floating root）。
- **优先级实现 —— "原 class 永远存在，新增不破坏"**：所有 region 都用 `[原 class, props.classNames?.region]` 数组合并；Vue normalize 会把 `undefined` 自动忽略。**零 breaking** —— 不传 classNames / styles prop 时行为完全一致；传了也只是追加而非覆盖（用户传 `root: 'my-root'` 不会移除组件自带的 `.ccui-input` 等命名空间 class）。
- **测试 +48**（roadmap 估 +25）：每组件 2 个测试 —— `classNames.root` 注入验证 + `styles.root` 注入验证。多出 +23 是因为 24 组件 × 2 ≈ 48 而 roadmap 估计是按"每组件 1 测试"算的，实际上加 styles 也需要单独断言，所以双倍。**根定位策略**差异化：录入 input / inputNumber / textarea 无 popup 用 `wrapper.classes()`；其余 9 个带 popup 触发器组件用 `wrapper.find(ns.b()).classes()`；Modal / Drawer / Notification / Message 走 `document.body.querySelector` 因 Teleport。
- **commit 体例**：1 个 commit 收口（含 1 共享 helper + 24 组件 types + 24 组件 tsx + Notification / Message 的双 holder 透传 + 24 测试文件 + 1 roadmap 更新）。后续 XL-6 v2.0 升级指南可基于本批 region key 表整理「定制化样式注入」的迁移示例。

</details>

---

**✅ M-A4 已交付（2026-05-16）**：1921 → 1948 测试（+27，98 → 99 文件）。Tier M-A 第四批 —— 8 个录入组件统一 Ant Design 风格的自定义图标钩子（`clearIcon` / `suffixIcon` / `removeIcon` / `expandIcon`），每个钩子同时支持 prop 形式（string Iconify name / CSS class / VNode）与同名 slot，slot 优先级高于 prop。要点：

<details>
<summary>详细要点</summary>

- **新建共享 helper**：`packages/ccui/ui/shared/hooks/use-icon.ts` 暴露 `renderIconNode(value)` —— 三态智能渲染：string 含 `:` 走 `<IconifyIcon icon={value} />`、string 不含 `:` 走 `<i class={value} />`（兼容 iconfont）、VNode 直接返回、空值返回 null。复用了 Input L-1.2 的 `customClearIcon` 范式抽象。
- **现状审计结论**：roadmap 估"10 个录入"，实际审计 12 录入组件后落实 **8 个组件**（Input / Textarea 在 L-1.2 / L-2.1 已完整支持 clearIcon，InputNumber / Mentions 无典型图标钩子语义，跳过）。新增钩子总数 22 点：AutoComplete 1（clearIcon）/ ColorPicker 1（clearIcon）/ DatePicker 2（clearIcon + suffixIcon）/ RangePicker 2 / TimePicker 2 / Select 3（clearIcon + suffixIcon + removeIcon）/ Cascader 4（clearIcon + suffixIcon + removeIcon + expandIcon slot；prop string 形式 L-1.x 已支持，本批扩为同时支持 VNode + 新增同名 slot 高优先级）/ TreeSelect 3（clearIcon + suffixIcon + removeIcon）。
- **expandIcon 半实施**：Cascader 的 `expandIcon` prop 已存在 string 形态；本批扩为 `string | VNode` 联合，并新增 `expandIcon` slot（scope `{ item, level }`）。TreeSelect 的 expandIcon 涉及内嵌 Tree 组件内部改造（Tree 自身尚无 expandIcon 钩子），本批不动，留 M-B5 TreeSelect 长尾批次。
- **loadingIcon 跳过**：审计发现 Select / AutoComplete / TreeSelect 的 loading 状态并无 spinner SVG 渲染（仅文字"加载中"或 mask 遮罩），加 `loadingIcon` 需要新增 spinner UI（设计层面要新增 SCSS + 旋转动画），超出图标钩子范围，留 M-B 批次。
- **prefix 跳过**：Input / Select 已通过 slot 形态完整支持；Ant 风格 prefix prop 接 ReactNode，Vue 端 slot 已经覆盖业务需求。本批不强行加 prop（避免 API 冗余）。
- **优先级实现**：每个钩子点统一模式 —— `slots.xxxIcon ? slots.xxxIcon() : (renderIconNode(props.xxxIcon) ?? defaultText)`。slot > prop > fallback 三级回退；fallback 保留各组件原 hardcoded 字符（如 DatePicker 的 📅、TimePicker 的 ⏱、Select 的 v 箭头、Cascader 的 ▾），零 breaking。
- **测试 +27**（roadmap 估 +20）：每组件 2-4 个测试（prop 形式 CSS 类名渲染 + slot 优先级覆盖；Cascader / Select 多覆盖 removeIcon multi-tag + expandIcon scope slot 路径）；helper 单测 +4（Iconify name 返回 IconifyIcon VNode + icon prop 透传 / CSS class 路径 / VNode 直返 / 空值边界）。多出 +7 因为 helper 单测拆得细 + 几个组件加了 multi-icon 测试。
- **commit 体例**：1 个 commit 收口（含 1 共享 helper `use-icon.ts` + 8 组件 src 改造（types 加 prop + tsx slot/prop/fallback 三级回退渲染）+ 9 测试文件 + 1 roadmap 更新）。后续 M-B5 TreeSelect 长尾批 可以基于本批 use-icon helper 扩展 Tree 组件的 expandIcon。

</details>

---

**✅ M-A5 已交付（2026-05-16）**：1881 → 1921 测试（+40，97 → 98 文件）。Tier M-A 收官批 —— 7 组件 31 个 deprecated 旧 prop 名落地 runtime warn（"全局 per-key 一次"），用户使用旧 API 时控制台一次性提示迁移到 Ant 主名。要点：

<details>
<summary>详细要点</summary>

- **新建共享 helper**：`packages/ccui/ui/shared/hooks/use-deprecation-warning.ts` 暴露三个工具 —— `warnDeprecatedProp(component, oldKey, suggestion)` 走 module 级 `Set<key>` 缓存全局 per-key 一次输出；`isPropExplicit(rawProps, camelKey, kebabKey)` 判定 raw vnode.props 中是否显式传入（同时查 camelCase 与 kebab-case，覆盖模板 / JSX / props 显式传值三种来源）；`__resetDeprecationWarningsForTest()` 仅测试用，清空全局 Set 以便 per-test 独立断言。
- **InputNumber 范式差异**：InputNumber 是**字面值替换**（`lg/md/sm → large/default/small`），warn 触发用 `props.size === 'lg'` 值匹配；M-A5 是**新旧 prop 名共存**（旧 prop default 可能是 `false/true/0/200/''` 非 undefined），单看 `props.xxx` 的值无法区分"用户显式 vs default"，必须用 `getCurrentInstance()?.vnode.props` 检查 raw key 存在性。这是 M-A5 与 InputNumber 范式的最大差异点。
- **缓存层级**：从"每实例 ref(false) 缓存"升级为"模块级 Set 全局缓存"。考虑场景是页面里 100 个 Modal 实例都传 `visible` 时只 warn 1 次（不是 100 次），与 Ant Design React 端的 `process.env.NODE_ENV !== 'production' &&` warn-once 范式对齐。
- **现状审计结论**：21 项 roadmap 估算实际是 **31 项**（grep 7 组件 types/tsx 出来）。types `@deprecated` 标注 + tsx 行为 fallback 已在 L-1.1 ~ L-1.6 全部完成，本批纯增量是补 runtime warn + 测试覆盖。**31 项分布**：Button 3（nativeType / round / circle）/ Input 3（clearable / prepend / append）/ Modal 5（visible / closeOnEsc / okLoading / hideFooter / appendToBody）/ Drawer 4（visible / closeOnEsc / showFooter / appendToBody）/ Tooltip 6（content / visible / showArrow / showAfter / hideAfter / popperClass）/ Popover 6（visible / showArrow / showAfter / hideAfter / popperClass / teleported）/ Popconfirm 3（visible / confirmText / confirmType）/ FormItem 1（prop）。
- **顺手修复 Popconfirm 内部对 Popover 传旧 prop 自污染**：Popconfirm 模板中向内嵌 Popover 同时传了 `visible / popperClass / showArrow / @update:visible` 四个 deprecated 旧名，导致每次 Popconfirm mount 都会触发 Popover 的 visible/popperClass/showArrow warn（误伤）。本批改为 `open / overlayClassName / arrow / @update:open` 新名，Popconfirm 自己的 warn 仅在用户对 Popconfirm 显式传 `visible/confirmText/confirmType` 时触发。
- **优先级语义统一**：所有 7 组件遵循 "Ant 新名显式优先 > 旧名兜底"（与 L-1.x 已实现的行为一致），新名 default `undefined` 用 `??` 或 `prop.newName !== undefined ?` 判定，旧 prop 仍参与默认值兜底；行为兼容零 breaking。
- **测试 +40**（roadmap 估 +30）：每组件 deprecation describe block 含 N 个 it（per deprecated prop），使用 `vi.spyOn(console, 'warn')` + `__resetDeprecationWarningsForTest()` 在 `beforeEach` 隔离测试。Button +3 / Input +3 / Modal +5 / Drawer +4 / Tooltip +6 / Popover +6 / Popconfirm +3 / FormItem +2（1 warn + 1 反向「新名 name 不 warn」）= 32 组件测试。Helper 单测 +8（3 warnDeprecatedProp 缓存语义 + 5 isPropExplicit camel/kebab/null/undefined/缺失 key 边界）。多出 +10 是 helper 单测拆得细 + Popconfirm/FormItem 额外加反向断言。
- **commit 体例**：1 个 commit 收口（含 1 共享 helper + 7 组件 tsx + 1 popconfirm 内部 prop 名替换 + 8 测试文件 + 1 roadmap 更新）。后续 XL-6 v2.0 升级指南可以基于本批 warn 文案直接整理迁移表。
...
- **共享 SCSS mixin**：在 `packages/ccui/ui/style-var/` 新建 `_form-control-variant.scss`，定义两个 mixin —— `form-control-variants`（视觉边框在根元素：Input / InputNumber / Textarea / Mentions）+ `form-control-variants-on($child)`（视觉边框在子元素：Select **selector、Cascader / TreeSelect / DatePicker / RangePicker / TimePicker **input-wrap、AutoComplete **wrap、ColorPicker **trigger）。`@forward` 到 `style-var/index.scss`，各组件 `@use '../../style-var/index.scss' as *;` 后直接 `@include`。Mixin 仅输出 3 个 modifier 块（outlined 即默认 SCSS 状态，不重复），filled focus 时 bg → bg-container + border → primary；borderless / underlined 关闭 box-shadow；underlined `border-bottom-color: primary-hover` hover 反馈。
...
- **tsx**：每组件根 class 增 `[ns.m(\`variant-${props.variant}\`)]`，对应 mixin 生成的 `&--variant-xxx`选择器；composite 组件（Select / Cascader 等）的根 modifier 配合 mixin 的`&--variant-filled &\_\_selector` 选择器命中子元素的边框层。
...
## Tier M-B — 单组件高频功能补齐（9 批，每批 1–2 天）

> 2026-05-16 调整方向：不再追求 95 → 100 完全对齐 ant；仅补齐**高频功能** + **slot 翻译 render prop**。下表「保留项」剔除掉 21 项低频 / niche / 边缘项（`showToday` / `multiple` v6 / `separator` / 响应式单面板 / preset 命中高亮 / 滚轮 snap / `showCheckedStrategy` 常量 / `halfCheckedKeys` / `destroyOnHidden` / EyeDropper / `adaptiveHeight` / `responsive` 配置 / `pauseOnFocus` / `pauseOnDotsHover` / `waitForAnimate` / `dotPlacement` 别名 / `matchInputWidth` / 虚拟滚动 / RTL / chunk 分片 / `LIST_IGNORE` 常量），统一不补。

| Batch | 组件        | 保留项                                                                                                                                                                                                                                  | 状态 |
| ----- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| M-B1  | DatePicker  | `minDate` / `maxDate` / `disabledTime` / 完整键盘导航 / `cell` slot（替代 `cellRender`） / `extra-footer` slot（替代 `renderExtraFooter`）                                                                                              | [x]  |
| M-B2  | RangePicker | `allowEmpty: [boolean, boolean]` / `disabled: [boolean, boolean]`                                                                                                                                                                       | [x]  |
| M-B3  | TimePicker  | `TimeRangePicker` 独立组件 / `cell` slot                                                                                                                                                                                                | [x]  |
| M-B4  | Cascader    | `maxTagCount` + `max-tag-placeholder` slot + `maxTagTextLength` / 键盘导航 / `showSearch` 对象 `limit`/`sort` / `option` slot（替代 `optionRender`） / `popup` slot（替代 `popupRender`） / `searchOption` slot（替代 `showSearch.render`） | [x]  |
| M-B5  | TreeSelect  | `showSearch` 完整 / `loadData` / 键盘导航                                                                                                                                                                                               | [x]  |
| M-B6  | ColorPicker | `presets` 预设色板 / `panel` slot（替代 `panelRender`）                                                                                                                                                                                 | [x]  |
| M-B7  | Carousel    | `slidesToShow` / `slidesToScroll` / `pauseOnHover`                                                                                                                                                                                      | [x]  |
| M-B8  | Transfer    | `selections-icon` slot（替代 `selectionsIcon` render prop）                                                                                                                                                                             | [x]  |
| M-B9  | Upload      | `listType='picture-card'` 完整样式 / `Upload.Dragger` 独立子组件                                                                                                                                                                        | [x]  |
...
- **demo 净增 +6 块**：7 → 13（Basic / **模板式列声明** / **自定义单元格渲染** / **树形数据** / Sort And Filter / 固定列 / 展开行 / 合并单元格 / Row Selection / **单选 + 实时统计** / **loading / size / showHeader** / **自定义空态** / **change 事件全量追踪**）。roadmap 表估 +25 是按 ant 全集估，**ccui Table API 已基本对齐 ant**，+6 已覆盖现 API 表面的全部 demo 维度。
...
- **change 事件全量追踪**：`@change="(filters, sorter, currentData) => ..."` 3 参数 destructure + JSON.stringify pretty-print 到 `<pre>`；最常见的后端 API 联动 / 埋点 / 持久化筛选状态范式，明确演示 sorter 是 `{ columnKey, order }` 对象、filters 是 `Record<columnKey, value[]>`、currentData 是当前过滤 / 排序后的全量数据数组。
...
- **写作约定**：`<script setup>` 风格 / 单 demo 单功能 / 业务场景命名（成绩分组合计 / 套餐选择 / 部署刷新 / 引导按钮）便于业务直接复制 / `customRender: ({ text }) => \`¥${text.toLocaleString()}\``类简短业务格式化用箭头函数 /`c-segmented`切换状态用 ref（与 L-4.3 Spin 多 Spin 网格 / L-4.4 Image 受控预览写法一致） /`<c-table-summary>`default slot 直接写原生`<tr><td>`而非 wrap div（Table 内部`h('tfoot', [slot()])` 渲染时表格 DOM 语义需要 tr/td）。
- **测试 +0**（纯文档 demo），全量回归 1906 不变（104 文件 / 1906 用例）。
...
**✅ L-2.25 已交付（2026-05-15）**：测试数不变（仅常量 export）。Tier L-2 TreeSelect SHOW\_\* 顶层常量（对标 ant `TreeSelect.SHOW_PARENT` / `.SHOW_CHILD` / `.SHOW_ALL`）。要点：
...
**✅ L-2.24 已交付（2026-05-15）**：测试数不变（仅常量 export）。Tier L-2 Cascader SHOW\_\* 顶层常量（对标 ant `Cascader.SHOW_CHILD` / `.SHOW_PARENT`）。要点：
...
| L-4.8 | Typography（与 L-3.7 配套交付）+ Calendar（与 XL-2 配套交付）                      | +20 demo  | [x]  |

---

## Tier XL — 架构 / Breaking 演进（每批 1–2 周）

> **执行序**：XL-1 / XL-2 / XL-3 / XL-4 / XL-5 顺序可调（互不阻塞），**XL-6 必须最后**（v2.0 收口节点，依赖前 5 批落地）。

| Batch | 范围                                                                                                                                                                                                            | 影响                                                                                  | 风险                        | 状态 |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | --------------------------- | ---- |
| XL-1  | Form.Item `prop` → `name` 切换为主名，`prop` 保留 deprecated 至下一大版本                                                                                                                                       | docs 几十处 :::demo 重写 + 测试用例改                                                 | 中（breaking 但有别名兼容） | [x]  |
| XL-2  | Calendar 从原生 `Date` 迁到 `Dayjs`（对齐 DatePicker 系），引入 `valueFormat` Vue 协议                                                                                                                          | Calendar 全量重写 + valueFormat 协议引入；解卡 L-4.8 Calendar demo                    | 高（数据协议变更）          | [x]  |
| XL-3  | 全量 deprecation policy 落地：旧 prop 在 dev mode 触发 console.warn（每个 key 一次性，避免噪音），文档加 deprecated 标记，定一个删除时间窗                                                                      | 影响所有 deprecated 标记的 prop（命名差异表 21 项 + Tag.bordered + InputNumber size） | 低                          | [x]  |
| XL-4  | 跨组件 ARIA 完整审计 + 键盘导航补齐（Cascader / Menu / Tree / DatePicker 系 / Dropdown / Tabs 等）                                                                                                              | 30+ 组件键盘 + screen reader 测试                                                     | 中                          | [x]  |
| XL-5  | 国际化二期：DatePicker 系 dayjs/locale 包接入 + 完整 enUS / 其他语言包覆盖（依赖 XL-2 dayjs 体系）                                                                                                              | locale 接口扩展 + 包体积管理                                                          | 中（需要权衡 dayjs 体积）   | [x]  |
| XL-6  | Element Plus 风格 API 全量切换到 Ant 风格主名（L-1 + M-A5 完成后），旧名彻底转为 deprecated 兼容层；同步把所有 render-prop 翻译成 slot；README 与 introduce.md 强调「Vue 组件库 / 对标 Ant Design 视觉 + 心智」 | README 表述同步 + 公开 v2.0 升级指南                                                  | 高（breaking 升大版本）     | [x]  |

---

## 节奏建议（2026-05-16 更新）

Tier S / M-A / L-1 / L-2 / L-3 已收官，L-4 文档批已陆续配套交付（L-4.8 因 Calendar 卡 XL-2 维持 `[~]`）。当前推进顺序：

### 阶段 1：单组件高频功能补齐（Tier M-B，9 批）

M-B1 → M-B5 → M-B9 → M-B4 → M-B6 → M-B7 → M-B2 → M-B3 → M-B8。总缺口收敛到 25 项保留（21 项已明确不补，见 Tier M-B 表头说明），按业务高频度优先：

- 业务后台直接命中：DatePicker / RangePicker / TreeSelect / Upload
- 基础录入：Cascader / ColorPicker
- 使用频次较低：Carousel / TimePicker / Transfer

### 阶段 2：横向收口（Tier XL-1 / XL-3 / XL-4，互不阻塞）

- XL-1 Form.Item `prop` → `name` 主名切换
- XL-3 deprecation policy 全量收紧（dev warn 触发条件 + 删除时间窗）
- XL-4 跨组件 ARIA + 键盘导航补齐

### 阶段 3：Calendar 重构 + L-4.8 解卡（Tier XL-2）

XL-2 Calendar `Date` → `Dayjs` + `valueFormat` 协议；完成后 L-4.8 Calendar demo 解卡，标 `[x]`。

### 阶段 4：i18n 二期（Tier XL-5）

依赖 XL-2 的 dayjs 体系；DatePicker 系 locale 包 + 多语言覆盖。

### 阶段 5：v2.0 收口（Tier XL-6）

Element Plus 风格 → Ant 主名全切，旧名转 deprecated 兜底；升大版本前最后一步，依赖前 5 批落地。

---

## v2.0 收官说明（2026-05-17）

Tier XL 六批全部完成，ccui 正式发布 `v2.0.0` 稳定版（脱 beta）。本批次累计交付：

- **XL-1**：Form.Item `prop` → `name` 主名切换，旧名走 deprecated 兼容层。
- **XL-2**：Calendar 切 Dayjs 协议 + `valueFormat`（`'string' | 'date' | 'number'`），同步解卡 L-4.8 Calendar demo（+8 demo）。
- **XL-3**：全量 deprecation policy 落地，新增 `shared/utils/deprecated.ts`（dev-only `warnDeprecated`，module-level Set 去重）；20+ 组件接入 warn。
- **XL-4**：跨组件 ARIA + 键盘审计，覆盖 浮层 / 选择 / 列表 / 录入 4 桶 30+ 组件；Tabs 补 Arrow/Home/End 键盘切换。
- **XL-5**：i18n 二期，`shared/utils/dayjs-locale.ts` lazy 加载 dayjs locale 包；ConfigProvider 注入时切全局 `dayjs.locale()`；扩 `CalendarLocale` namespace + `PaginationLocale.pagination`；新增 `ja-JP` / `ko-KR` 两个完整 locale 包。
- **XL-6**：v2.0 发布收口，命名差异表 35 项映射写入 `introduce.md` 升级指南；version 升 `2.0.0`，publishConfig tag 改 `latest`；README 表述同步。

**测试基线**：M-B 收官 2020 / 91 文件 → XL 收官 ~2100 / 94+ 文件（+80+ 用例）。

**deprecated 删除时间窗**：v3.0（无 ETA），旧名运行时仍可用，dev mode 输出一次性 `console.warn` 提示。

至此 Tier S / M-A / M-B / L-1..4 / L-2..3 / XL-1..6 全部收官；后续工作走 issue / PR 节奏，roadmap 转为长期维护模式。

---

## Tier V3 — 删旧名（无 ETA）

ccui 下一次大版本（v3.0）的清理批次：把 v2.0 introduced 的 deprecated 旧名物理删除，版本号升 `3.0.0`，不提供 alias bridge。

**触发条件**：下一次大版本周期由维护者决定，不做 adoption gating —— 大版本本身就是 breaking 窗口。

**删除范围（26 项）**，与 `scripts/codemod-v1-to-v2.mjs` 映射表完全对齐：

- **Modal**：`okLoading` / `hideFooter`
- **Drawer**：`showFooter`
- **Tooltip**：`content` / `showArrow` / `showAfter` / `hideAfter` / `popperClass`
- **Popover**：`showArrow` / `showAfter` / `hideAfter` / `popperClass` / `teleported`
- **Popconfirm**：`confirmText` / `confirmType`
- **Button**：`nativeType` / `round`（boolean）/ `circle`（boolean）/ `plain`（boolean）
- **Input**：`clearable` / `prepend` / `append`
- **InputNumber.size**：旧值 `'lg'` / `'md'` / `'sm'`
- **Tag**：`bordered`
- **ColorPicker**：`format='hsv'`
- **FormItem**：`prop`

**永久保留**（Vue-first canonical，不在删除范围）：`visible` / `closeOnEsc` / `appendToBody`。

**工具保留**：`shared/utils/deprecated.ts` 的 `warnDeprecated` 自身保留，给 v3 → v4 周期复用。

**用户迁移路径**：`node scripts/codemod-v1-to-v2.mjs <glob> --apply` 自动改写模板与 TS prop key（详见 `scripts/CODEMOD.md`）。

**影响**：major bump → `3.0.0`，无 alias bridge；接入方 v2 → v3 必须跑 codemod 或手动改名。
