...
**deprecated 处理记录**：v2 beta 阶段（2.0.1-beta.3 / beta.4）已把 v2 引入的 Ant 风格新名整批清掉（按"哪种更顺 Vue 习惯"做反向决策），不再延续到 v3。详见 commit `2089965`（首批 6 组件 9 项）与本次清理 commit 串（A-G 8 task，对应 commit `b91b181..` 起）。
